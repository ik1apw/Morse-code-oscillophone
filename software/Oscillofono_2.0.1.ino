/* =====================================================
   OSCILLOFONO DIDATTICO CW – Versione 2.0.1
   Autore: IK1APW (Emanuele Rossi)
   Anno: 2026

   Funzioni principali:
   - Generatore CW con DDS sinusoidale
   - Decoder Morse in tempo reale
   - Keyer paddle iambico non-blocking
   - Regolazione frequenza e velocita' (WPM)
   - Memorizzazione MODE in EEPROM
   - Interfaccia encoder + LCD 16x2 I2C
   ===================================================== */
...
//                    AUDIO DDS
// =====================================================
// Generazione audio tramite DDS a 8 bit:
// - Timer2 genera il campionamento (ISR)
// - Timer1 produce il PWM audio (da filtrare) -> sinusoide
// - Envelope ATT/REL evita "click" all'inizio/fine tono
...
ISR(TIMER2_COMPA_vect) {
  // ISR audio:
  // - genera la sinusoide DDS
  // - applica inviluppo anti-click (attack/release)
  // - aggiorna PWM audio
  if (keyState) {
    if (amp < AMP_MAX) {
...
// Tabella di decodifica Morse:
// converte sequenza di "." e "-" in carattere ASCII.
// Se non riconosciuto ritorna '#'.
...
// Keyer iambico non bloccante:
// gestisce DIT/DAH con timing automatico (WPM)
// usa una piccola macchina a stati (MARK/GAP) senza delay()
...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("by IK1APW @2026");
  lcd.setCursor(0, 1);
  lcd.print("Oscillofono 2.0.1");
  delay(2000);
  lcd.clear();
...
// LOOP principale:
// gestisce UI, encoder e la modalita' selezionata.
// Tutto il timing e' non-blocking (niente delay in loop).
